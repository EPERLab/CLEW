...
def set_txt_list( model_to_run ):
    ...
    for e in range( len( txt_list ) ):
        structural_list.append( 'STRUCTURE_OSEMOSYS_CR_ENERGY.xlsx' )
...
def data_processor( case, model_to_run_gen, sce_gen ):
    ...
    if model_to_run_gen == 'energy' and sce_gen == 'BAU':
        ini_line = 7529078 + 2
        end_line = 15008836
    
    if model_to_run_gen == 'energy' and sce_gen == 'SR15':
        ini_line = 923910+2
        end_line = 1831072
    #
...
def main_executer( n1 ):
    ...
    #
    file_aboslute_address = os.path.abspath("run_model_mathprog.py")
    file_adress = re.escape( file_aboslute_address.replace( 'run_model_mathprog.py', '' ) ).replace( '\:', ':' )
    #
    print('$$')
    print( txt_list )
    print('$$')
